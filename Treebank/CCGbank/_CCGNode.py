...
class CCGNode(Node):
    ...
    def isProduction(self, **kwargs):
        ...
        if isinstance(selfType, str):
            selfType = ccg.category.from_string(selfType)
        ...
        for nodeType, specified in kwargs.items():
            ...
            if isinstance(specified, str):
                specified = ccg.category.from_string(specified)
            if not specified.exact_eq(node.label):
                return False
        return True
    ...
    def move(self, destination, headIdx):
        ...
        if destination is self.sibling():
            print [str(i) for i in sorted([self, destination])]
            print destination
